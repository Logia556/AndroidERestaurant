...
class BasketAdapter(
    ...
    private val basketCellClickListener: BasketCellClickListener
) : RecyclerView.Adapter<BasketAdapter.BasketItemsViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BasketItemsViewHolder {
        return BasketItemsViewHolder(
    ...
    override fun onBindViewHolder(holder: BasketItemsViewHolder, position: Int) {
        ...
        holder.dishPrice.text =
            "${items[position].dish.prices[0].price.toFloat() * items[position].itemCount} €"
        ...
        holder.dishQuantity.text =
            "${items[position].itemCount} * ${items[position].dish.prices[0].price.toFloat()}"
    }
    ...
    class BasketItemsViewHolder(basketCellBinding: BasketCellBinding) :
        RecyclerView.ViewHolder(basketCellBinding.root) {
